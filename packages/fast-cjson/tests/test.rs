...
fn raw_value() {
    ...
    let parsed: TestValue<'_> = serde_json::from_slice(&encoded).unwrap();
    let encoded = encode!(parsed).unwrap();

    assert_eq!(
        br#"{"a":1,"b":2,"c":{"a":null,"h":{"h":-5,"i":3},"x":{}},"nested":{"bad":true,"good":false}}"#,
        encoded.as_slice(),
        "{}",
...
fn accept_raw_integer() {
    let raw_number: serde_json::Number = serde_json::from_str("8").unwrap();
...
#[test]
fn reject_raw_float() {
    let raw_number: serde_json::Number = serde_json::from_str("8.0").unwrap();
    let raw_number_small: serde_json::Number = serde_json::from_str("12.3e+11").unwrap();
    let raw_number_large: serde_json::Number = serde_json::from_str("13.12E+161").unwrap();
